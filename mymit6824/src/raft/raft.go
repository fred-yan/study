...
func (rf *Raft) sendOnePeerAppendEntries(server int, args *AppendEntriesArgs, reply *AppendEntriesReply) bool{
	...
	Logger.Printf("id %d finished send log or heartbeat to id %d and return %s", rf.me, server, ok)
	return ok
}
...
func (rf *Raft) beginElection() {
	...
	Logger.Printf("id %d begin election", rf.me)
...
const LeaderPeerTickInterval = 20 * time.Millisecond
...
func (rf *Raft) startLeaderPeerProcess(peerIndex int) {
	...
	for {
		...
		case currentTime := <-ticker.C: // If traffic has been idle, we should send a heartbeat
			Logger.Printf("id %d start a ticker to server %d", rf.me, peerIndex)
...
func (rf *Raft) startLocalApplyProcess(applyChan chan ApplyMsg) {
	rf.mu.Lock()
	Logger.Printf("id %d start commit process - last log applied %d", rf.me, rf.lastApplide)
	rf.mu.Unlock()

	for {
		rf.mu.Lock()
		if rf.commitIndex >= 0 && rf.commitIndex > rf.lastApplide {
			startIndex := rf.lastApplide + 1
			endIndex := rf.commitIndex

			if endIndex >= 0 && endIndex >= startIndex {
				entries := make([]LogEntry, endIndex-startIndex+1)
				copy(entries, rf.log[startIndex:endIndex+1])
				Logger.Printf("id %d commit from %d to %d , and entry %s ", rf.me, rf.lastApplide, rf.commitIndex, entries)

				for _, entry := range entries {
					applyChan <- ApplyMsg{Index:entry.Index+1, Command:entry.Command}
				}
				rf.lastApplide += len(entries)
			}
			rf.mu.Unlock()
		} else {
			rf.mu.Unlock()
